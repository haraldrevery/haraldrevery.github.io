...
5. [Asset Guidelines & File Sizes](#-asset-guidelines--file-sizes)
...
## 📦 Asset Guidelines & File Sizes

Following these guidelines will ensure fast page loads and good user experience across devices.

### 🖼️ Images

#### **Blog Post Thumbnails** (for notebook cards)
- **Dimensions:** 1200×675px (16:9 aspect ratio)
- **Format:** JPEG or WebP
- **Quality:** 80-85% (JPEG) / 75-80% (WebP)
- **Target file size:** 80-150 KB
- **Why:** These appear in grid layouts and need to load quickly. The aspect ratio matches the `aspect-video` class used in templates.

**How to optimize:**
```bash
# Using ImageMagick
convert input.jpg -resize 1200x675^ -gravity center -extent 1200x675 -quality 82 thumbnail.jpg

# Using cwebp for WebP
cwebp -q 78 input.jpg -o thumbnail.webp
```

#### **Full-Resolution Blog Images** (inside articles)
- **Max width:** 2400px (covers high-DPI displays up to 1200px logical width)
- **Format:** JPEG or WebP
- **Quality:** 85-90% (JPEG) / 80-85% (WebP)
- **Target file size:** 150-400 KB depending on content
- **Why:** Blog posts use the `.prose` container which has max-width constraints. Anything wider than 2400px is wasted bandwidth.

**How to optimize:**
```bash
# Resize if needed and optimize
convert input.jpg -resize 2400x2400\> -quality 87 -strip optimized.jpg
```

#### **Background Images** (like `bgtexture.jpg`)
- **Dimensions:** Match your design needs, typically 1920×1080 or larger
- **Format:** JPEG (backgrounds rarely need transparency)
- **Quality:** 75-80% (these are often subtle textures)
- **Target file size:** 100-250 KB
- **Consider:** Using CSS `background-size: cover` so you only need one size

#### **Open Graph Image** (`opengraphimg.jpg`)
- **Dimensions:** 1200×630px (Facebook/Twitter recommended)
- **Format:** JPEG
- **Quality:** 85%
- **Target file size:** 100-200 KB
- **Why:** This appears when sharing links on social media. Exact dimensions matter for proper cropping.

#### **Favicons**
- `favicon.ico`: 32×32px, <10 KB
- `favicon-96x96.png`: 96×96px, <15 KB
- `apple-touch-icon.png`: 180×180px, <25 KB
- `favicon.svg`: Vector, <5 KB (if using)

**General Image Optimization Tips:**
- Always strip EXIF metadata (`-strip` flag in ImageMagick)
- Use progressive JPEGs for images >10KB
- Consider WebP format (25-35% smaller than JPEG with same quality)
- Use tools like [Squoosh](https://squoosh.app/) or [TinyPNG](https://tinypng.com/) for manual optimization
- Lazy-load images below the fold using `loading="lazy"` attribute

### 🎵 Audio Files

#### **Music Tracks** (in `/music/mp3/`)
- **Format:** MP3
- **Bitrate:** 192-256 kbps (CBR or VBR)
  - 192 kbps: Good balance for streaming, ~2.3 MB per minute
  - 256 kbps: Higher quality, ~3.1 MB per minute
  - 320 kbps: Overkill for web (most users won't hear the difference)
- **Sample rate:** 44.1 kHz (CD quality)
- **Channels:** Stereo
- **Target file size:** 7-10 MB for a 3-4 minute track @ 256 kbps
- **Why:** These stream directly in the browser. Higher bitrates increase load time without significant quality improvement.

**Recommended encoding settings:**
```bash
# Using ffmpeg to re-encode
ffmpeg -i input.wav -codec:a libmp3lame -b:a 256k -ar 44100 output.mp3

# VBR (Variable Bit Rate) - better quality/size ratio
ffmpeg -i input.wav -codec:a libmp3lame -q:a 2 -ar 44100 output.mp3
# -q:a 2 is roughly equivalent to 190 kbps average
```

**Alternative formats to consider:**
- **AAC** (`.m4a`): 20-30% better compression than MP3 at same quality
  - Use 192-224 kbps AAC instead of 256 kbps MP3
  - Not all browsers support, but most modern ones do
- **Opus** (`.opus`): Best quality-to-size ratio for web
  - Use 128-160 kbps Opus = 256 kbps MP3 quality
  - Excellent browser support (all modern browsers)

**Storage estimates for a typical music page:**
- 8 tracks × 3.5 minutes × 256 kbps = ~70 MB total
- 8 tracks × 3.5 minutes × 192 kbps = ~52 MB total
- Consider hosting on a CDN or streaming service for better performance

### 📝 Fonts

#### **Web Fonts** (in `/fonts/`)
- **Format:** WOFF2 (Web Open Font Format 2)
- **Target file size per font:** 20-60 KB
  - Latin-only subset: 15-25 KB
  - Latin + Extended: 30-60 KB
- **Why:** WOFF2 has best compression (30% smaller than WOFF) and is supported by all modern browsers

**Font optimization:**
- Only include character subsets you need (Latin, not entire Unicode)
- Subset fonts using [glyphhanger](https://github.com/zachleat/glyphhanger) or [Fonttools](https://github.com/fonttools/fonttools)
- Use `font-display: swap` in CSS to prevent invisible text during load

```css
@font-face {
  font-family: 'HaraldReveryText';
...
- **Minified versions** (`main.css`, `prose.css`):
  - Typical size: 50-200 KB (varies based on classes used)
  - Target: <150 KB for main.css, <100 KB for prose.css
  - These should be compressed with gzip/brotli by your web server (reduces to 10-20 KB)

- **Unminified versions** (`main_max.css`, `prose_max.css`):
  - Development only - don't deploy to production
  - Can be 500 KB - 2 MB (readable formatting)

**Server compression:**
Most web servers automatically compress CSS with gzip/brotli. A 150 KB minified CSS file compresses to ~15-20 KB when served with gzip.

### 📊 Total Site Size Recommendations

**Initial page load budget (homepage):**
- HTML: <50 KB
- CSS: <150 KB (uncompressed) / ~20 KB (compressed)
- JavaScript: <100 KB total (Alpine + GLightbox + custom)
- Fonts: <100 KB total (2 font files)
- Images above the fold: <300 KB
- **Total:** Aim for <600 KB uncompressed, ~200-300 KB compressed

**Music page (including audio):**
- Base assets: ~600 KB (HTML/CSS/JS/fonts)
- 8 audio tracks @ 256 kbps: ~70 MB
- **Note:** Audio streams progressively, not loaded upfront

**Blog post page:**
- Base assets: ~600 KB
- Post images: 150-400 KB each
- **Total:** Aim for <2 MB per post

### 🚀 Performance Testing Tools

Use these to verify your optimizations:
- **[Google PageSpeed Insights](https://pagespeed.web.dev/)** - Overall performance score
- **[GTmetrix](https://gtmetrix.com/)** - Detailed waterfall analysis  
- **[WebPageTest](https://www.webpagetest.org/)** - Advanced testing with different locations/devices
...
2. **Add cache headers** for static assets (CSS, JS, images, fonts)
3. **Use a CDN** for global distribution if your audience is international
4. **Lazy load images** below the fold: `<img loading="lazy" ... >`
5. **Preload critical assets** (already in your templates):
   ```html
