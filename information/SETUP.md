...
- hand-written static pages, listed in that file's frontmatter. Their
  `<lastmod>` is the date their CONTENT last changed: each entry lists its
  `sources`, `eleventy_njk/sitemap.11tydata.js` fingerprints them, and
  `eleventy_njk/sitemap_dates.json` (committed) remembers the fingerprint and
  the date it first changed. Formatting, line endings and nav/footer edits do
  not count; file modification times are not used at all (git does not keep
  them, so they said nothing about content).
...
`staticPages` list, with its `sources`** — Eleventy ignores those pages, so it
cannot discover them on its own. Commit `eleventy_njk/sitemap_dates.json`
together with the rebuilt `sitemap.xml`: it is what keeps the dates the same on
every machine.
...
The live headers are set by Cloudflare dashboard rules. `_headers` below is a
ready-to-use copy that is **not in effect** on GitHub Pages — read the STATUS
block at its top.
...
The site is served by **GitHub Pages** (this repo's root, custom domain from
...
3. **Cloudflare sits in front of GitHub Pages** and owns the redirects and
   security headers (see Deploying) — not "your web hosting".
