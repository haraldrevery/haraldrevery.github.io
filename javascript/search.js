...
(() => {
    ...
    document.querySelectorAll("[data-search]").forEach((box, n) => {
        ...
        // The keyboard highlight is an inline style, NOT a Tailwind class: see the
        // note at the top — /javascript/ isn't scanned, so "bg-neutral-300" and
        // "dark:bg-neutral-700" were never emitted into main.css (only their
        // hover:/focus: variants are, from search.njk) and toggling them painted
        // nothing. The two vars below ARE in main.css, and the media query mirrors
        // Tailwind's default dark: variant (prefers-color-scheme, no .dark class).
        const dark = window.matchMedia("(prefers-color-scheme: dark)");
        const HL = () => dark.matches ? "var(--color-neutral-700)" : "var(--color-neutral-300)";

        const paint = () => list.querySelectorAll("a").forEach((a, i) => {
            a.style.backgroundColor = i === active ? HL() : "";
