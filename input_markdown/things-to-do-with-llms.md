...
Anyway, instead of speculating I will focus on naming some cool projects to play around with and why I think they are worth doing. The current (available) models performances are pretty good for these types of projects.
...
In contrast of having a static website that is your digital presence, I think it's also important to bring what is digital to the real world. 

In the last couple of decades (well, as long I have been alive) there has been a push to store everything digitally, despite its technology being really fragile. HDDs, SSDs and CDs can barely store something reliably for 10 years, while alternatives such as LTO Tape and M-DISCs still needs fragile and expensive hardware to be used, in turn also requires its own maintenance. As humans, it's really dangerous to rely on storing information in a format that requires advanced electronics and logic to extract said information. Paper has proven to be the most durable and cost effective medium. Books are simply battle tested and has stood the test of time for storing information. Books are the only physical medium that can guarantee information being stored between two generations. Just keep it away from fire and water, it will lasts for ~50 years (provided it's acid-free paper and some quality ink to guarantee easy readability) and you only need light and reading skills to extract information from the medium. 
...
This one is making a lot noise, but personally I think making simple versions of commercial software that can run on any hardware is convenient. Even if they might contain jank, it's at least *my own jank* that I know how to work around. Every commercial software out there is made to synchronise to a server (spyware) and require you to have a paid subscription for certain features or even just opening up your files (let's be real here, it's ransomware at this point). A simple app that just changes a files content without enforced Internet connection, ads, and payments is how computers are supposed to work. LLMs can finally make computers usable again. 
