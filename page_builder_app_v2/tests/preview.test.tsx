...
describe("buildPreview", () => {
  ...
  test("wraps exactly the exported fragment in the generated shell", async () => {
    ...
    const exported = exportText({
      data, config, siteUrl: SITE, slug: "galdhopiggen",
      heroHtml: renderExportHero(data),
      contentHtml: renderExportContent(data),
    });

    // Preview and export do not produce the same bytes: export writes a body
    // fragment for Eleventy to wrap, preview wraps that same fragment in
    // shell.html (generated by eleventy_njk/_builder_shell.njk from the same
    // base.njk and post_chrome.njk). The invariant here is that the preview
    // contains the exported body verbatim; site-build.test.tsx checks that the
    // page around it matches the published one.
    const fragment = exported.slice(exported.indexOf("\n---\n") + 5);
    expect(preview).toContain(fragment.trim());
    expect(preview).toContain("<!DOCTYPE html>");
    expect(preview).toContain("<footer");
  });

  test("shows the layout's header and ending exactly once", async () => {
    // They come from the shell (post_chrome.njk), not from the fragment, so
    // this is the preview's half of "one of each"; site-build.test.tsx checks
    // the published page.
    const html = await buildPreview(mk([text("t1", "Body.")]), shell, SITE);
    expect(html.match(/NOTEBOOK FRONT PAGE/g)).toHaveLength(1);
    expect(html.match(/← Back to Notebook/g)).toHaveLength(1);
    expect(html).not.toContain("pb:header");
  });

  test("refuses a shell that predates the page header and ending", async () => {
    // The shell is regenerated by the site build. Previewing with one older
    // than this app would show no header and no ending and say nothing.
    const old = shell.replace(/<!--pb:header-->[\s\S]*?<!--\/pb:header-->/, "");
    await expect(buildPreview(mk([text("t1", "Body.")]), old, SITE)).rejects.toThrow(/Run the Eleventy build/);
  });
