...
/// incompatible (v1 stores {version,meta,blocks}, v2 stores Puck Data), and
/// `shell.html` is *written* by adopt_shell_region, so sharing it would mean
/// two apps mutating one file. Each app's shell is kept honest independently
/// by check_shell_freshness, which diffs against REFERENCE_PAGE, not the other
/// app. Renaming this folder = change this one line and rebuild.
...
fn load_shell_and_reference(root: &Path) -> Result<(String, String), String> {
    let shell_path = root.join(APP_DIR).join("shell.html");
