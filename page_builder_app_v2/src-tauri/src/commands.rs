...
/// deleted.
///
/// Exports carry no page furniture: post_body.njk adds the date/<h1>/back-link
/// header and the closing date rule + "← NOTEBOOK FRONT PAGE" link, and
/// eleventy.config.js refuses a body that brings its own ending. A page with a
/// hero exports `header: false`, because the hero carries its own title and
/// back link (see "WHO OWNS WHAT" in src/export/export.ts).
///
...
/// preview, "Export HTML…" and the editor's page header — never for Export. Do
/// not edit it; edit eleventy_settings/nav.njk, footer.njk, base.njk or
/// post_chrome.njk. Renaming this folder = change this one line, the permalink
/// in that template, and rebuild.
...
// eleventy_njk/_builder_shell.njk from the same layouts every post uses, so it
// is refreshed on every site build and cannot drift. It is never used for
// Export; exported pages are body fragments wrapped by post_body.njk and
// base.njk at build time. A shell OLDER than this app (no header region) is
// reported by the frontend, not here: see checkShell in src/export/export.ts.
//
