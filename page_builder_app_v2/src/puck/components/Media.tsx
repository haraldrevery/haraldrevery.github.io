...
export interface VideoProps {
  src: string;
  poster: string;
  /// The heading in the glass panel, under the player.
  title: string;
  /// The description paragraph under the title. Named `caption` because that
  /// is the key saved projects already store it under.
  caption: string;
  spacing: Spacing;
  puck?: PuckContext;
}
...
/*
 * The player in the site's frosted .preview-card, with the title and the
 * description underneath (input.css "Video card") — the one look this block
 * has, and the markup stress_test.html's video block is written against.
 *
 * There used to be a panel-off variant, a bare <figure>. It was dropped: its
 * <figcaption> sat outside any .prose wrapper, and figcaption is only styled
 * inside .prose, so it published as unstyled body text. A project saved with
 * `panel: false` still carries that key; it is ignored and the block renders
 * the panel.
 */
export function Video({ src, poster, title, caption, spacing, puck }: VideoProps) {
  if (!src) {
      ...
      <figure className="preview-card video-card">
        ...
        {heading || text ? (
          <figcaption className="video-card__caption">
            ...
            {text && <p className="video-card__text">{text}</p>}
          </figcaption>
        ) : null}
      </figure>
    </BlockShell>
