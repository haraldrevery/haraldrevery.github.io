...
function HueWheel({ hue, onChange, size = 240, harmonyHues = [] }) {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < steps; i++) {
      ...
      ctx.closePath();
      const { r, g, b } = ColorUtil.hsvToRgb(i, 1, 1);
  ...
  const handleMove = (e) => { if (dragging.current) handle(e); };
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size }}>
      ...
      {/* Connector polygon between harmony markers */}
      {markers.length > 1 && (
        <svg
      ...
      {/* Main current-hue indicator (larger) */}
      <div style={{
        position: 'absolute', left: ix - 10, top: iy - 10, width: 20, height: 20,
...
function SVSquare({ hue, sat, val, onChange, size = 180 }) {
  const ref = useRef(null);
  const dragging = useRef(false);
  ...
  useEffect(() => {
    ...
    ctx.fillStyle = gx; ctx.fillRect(0, 0, size, size);
    const gy = ctx.createLinearGradient(0, 0, 0, size);
    gy.addColorStop(0, 'rgba(0,0,0,0)');
    gy.addColorStop(1, 'rgba(0,0,0,1)');
    ctx.fillStyle = gy; ctx.fillRect(0, 0, size, size);
  }, [hue, size]);
  ...
  const px = sat * size, py = (1 - val) * size;

  return (
    <div style={{ position: 'relative', width: size, height: size }}>
      <canvas
        ref={ref}
        style={{ width: size, height: size, cursor: 'crosshair', display: 'block', borderRadius: 2, touchAction: 'none' }}
        onPointerDown={handleDown}
        onPointerMove={handleMove}
        onPointerUp={handleUp}
        onPointerCancel={handleUp}
      />
      <div style={{
        position: 'absolute', left: px - 8, top: py - 8, width: 16, height: 16,
        borderRadius: '50%', border: '2px solid #fff', boxShadow: '0 0 0 1px #000', pointerEvents: 'none'
      }} />
    </div>
  );
}
...
function HueStrip({ hue, onChange }) {
  ...
  return (
    <div ref={ref}
      ...
      onPointerCancel={handleUp}
      style={{
        position: 'relative', height: 14, borderRadius: 2,
        background: 'linear-gradient(to right, hsl(0,100%,50%), hsl(60,100%,50%), hsl(120,100%,50%), hsl(180,100%,50%), hsl(240,100%,50%), hsl(300,100%,50%), hsl(360,100%,50%))',
        cursor: 'crosshair', border: '1px solid rgba(0,0,0,0.12)', touchAction: 'none'
      }}>
      <div style={{
        position: 'absolute', left: `${hue / 360 * 100}%`, top: -2, width: 8, height: 18,
        marginLeft: -4, background: '#fff', border: '1px solid #000', borderRadius: 1, pointerEvents: 'none'
      }} />
    </div>
  );
}

Object.assign(window, { HueWheel, SVSquare, HueStrip });
