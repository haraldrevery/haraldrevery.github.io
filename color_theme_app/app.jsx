...
const withStopId = (c) => (c && c.id ? c : { ...c, id: genStopId() });
...
const safeSet = (k, v) => { try { localStorage.setItem(k, v); } catch (_) { /* ignore */ } };

// --------- Custom dropdown (so options use the brand font) ---------
function Dropdown({ value, onChange, options, leading, width }) {
  ...
  return (
    <div className={'dd' + (open ? ' open' : '')} ref={wrap} style={width ? { minWidth: width } : null}>
      <button
        type="button"
        ...
        {leading && <span className="dd-leading">{leading}</span>}
        <span className="dd-current">{current ? current.label : ''}</span>
        <svg className="dd-caret" width="10" height="10" viewBox="0 0 10 10" fill="none" aria-hidden="true">
...
function CopyRow({ label, value }) {
  ...
  return (
    <div className="copy-row" onClick={copy} title="Click to copy">
      <span className="copy-label">{label}</span>
      <span className="copy-value">{value}</span>
      <span className="copy-cta">{copied ? 'copied' : 'copy'}</span>
    </div>);
...
function ColorOutputs({ rgb, format, setFormat }) {
  const f = ColorUtil.format(rgb);
  const valueByKey = {
    hex: f.hex, rgb: f.rgb, hsl: f.hsl, hsv: f.hsv, oklch: f.oklch, cmyk: f.cmyk
  };
  ...
  return (
    <div className="outputs">
      ...
      <button className="readout" onClick={copy} title="Click to copy">
        <span className="readout-value">{current}</span>
        <span className="readout-cta">{copied ? 'copied' : 'copy'}</span>
      </button>
      {f.name &&
      <div className="readout-name">
          <span className="rn-label">named</span>
          <span className="rn-value">{f.name}</span>
        </div>
      }
    </div>);
...
// --------- Named colors panel ---------
function NamedColors({ onPick, currentHex }) {
  const [q, setQ] = useS('');
  const list = useM(() => {
      ...
      <div className="named-grid">
        ...
        <button
          ...
          title={`${n} ${h}`}>
            ...
            <span className="named-sw" style={{ background: h }} />
            <span className="named-text">
              <span className="named-name">{n}</span>
              <span className="named-hex">{h.toUpperCase()}</span>
            </span>
          </button>
        )}
...
function HarmonyChip({ color, format }) {
  const [copied, setCopied] = useS(false);
  ...
  return (
    <button
      ...
      <span className="harm-sw checker" style={{ '--swatch-color': css }}>
        <span className="harm-copied">copied</span>
      </span>
      <span className="harm-hex">{label}</span>
    </button>);
}

function Harmonies({ hsv, alpha, mode, setMode, count, setCount, spread, setSpread, harmonyHues, format, onPick, onSave }) {
  const colors = useM(() => {
  ...
  return (
    <section className="panel">
      <header className="panel-h">
        ...
        <div className="seg">
          <button className={'seg-btn' + (mode === 'analogous' ? ' on' : '')} onClick={() => setMode('analogous')}>Analogous</button>
          <button className={'seg-btn' + (mode === 'split' ? ' on' : '')} onClick={() => setMode('split')}>Split-comp</button>
        </div>
      </header>

      {mode === 'analogous' &&
      <div className="row-2">
          <label className="lbl"><span>Count</span><span className="lbl-val">{count}</span></label>
      ...
      <div className="harm-row">
        {colors.map((c, i) => (
          <HarmonyChip key={i} color={c} format={format} />
        ))}
      </div>
      <div className="row-2">
        <label className="lbl">
          <span>{spreadLabel}</span>
          <span className="lbl-val">{shownSpread}°</span>
        </label>
        <input
          type="range" min={5} max={maxSpread} value={Math.min(spread, maxSpread)}
          onChange={(e) => setSpread(Number(e.target.value))} />
        
      </div>
      <div className="grad-actions">
        <button className="btn btn-ghost" onClick={() => onSave && onSave({
          type: 'harmony',
          colors,
          mode,
          count,
          spread: shownSpread,
          baseHsv: { h: hsv.h, s: hsv.s, v: hsv.v },
          alpha: alpha == null ? 1 : alpha
        })} title="Save harmony to library (remembers mode + spread)">☆ save palette</button>
      </div>
    </section>);
...
// --------- Gradient generator ---------
function Gradient({ currentHex, currentRgb, format, stops, setStops, onSave }) {
  ...
  const setStop = (id, raw) => {
    ...
    if (c) setStops((s) => s.map((x) => x.id === id ? { ...c, id: x.id } : x));
  };
  const commitStop = (id) => {
    setDrafts((d) => {
      const { [id]: _, ...rest } = d;
  ...
  const addStop = () => setStops((s) => [...s, withStopId({ ...currentRgb })]);
  const removeStop = (id) => setStops((s) => s.length > 2 ? s.filter((x) => x.id !== id) : s);

  const useCurrent = () => {
    if (!activeId) return;
    setStops((s) => s.map((x) => x.id === activeId ? { ...currentRgb, id: x.id } : x));
    setDrafts((d) => {const { [activeId]: _, ...rest } = d;return rest;});
  };
    ...
    <section className="panel">
      ...
      <div className="grad-stops">
        {stops.map((c, i) => {
          ...
          activeId === id ? ' on' : '') + (
          dragId === id ? ' dragging' : '') + (
          overId === id && dragId !== null && dragId !== id ? ' over' : '');
          return (
            <div
              key={id}
              className={cls}
              ...
              <span className="stop-handle" title="Drag to reorder" aria-hidden="true">⋮⋮</span>
              <span className="stop-sw checker" style={{ '--swatch-color': css }} />
              <input
                type="text"
                ...
                placeholder={
                stopFormat === 'hex' ? '#000000' :
                stopFormat === 'rgb' ? 'R, G, B' :
                stopFormat === 'hsl' ? 'H, S%, L%' :
                stopFormat === 'hsv' ? 'H, S%, V%' :
                stopFormat === 'oklch' ? 'L C H' :
                'C%, M%, Y%, K%'
                }
                spellCheck={false} />
              
              <button className="x" onClick={(e) => {e.stopPropagation();removeStop(id);}} title="remove">×</button>
            </div>);

        })}
        <button className="add-stop" onClick={addStop}>+ stop</button>
        <button className="add-stop" onClick={useCurrent} title="Replace selected stop with current color">set from current</button>
      </div>

      <div className="row-2">
        <label className="lbl"><span>Steps</span><span className="lbl-val">{steps}</span></label>
        <input type="range" min={2} max={24} value={steps} onChange={(e) => setSteps(Number(e.target.value))} />
      </div>

      <div className="grad-swatches">
        {swatches.map((c, i) => {
          const hex = ColorUtil.rgbToHex(c.r, c.g, c.b, c.a).toUpperCase();
          const css = ColorUtil.toCss(c);
          const label = ColorUtil.formatRgbAs(c, stopFormat);
          return (
            <button key={i} className="grad-sw"
            onClick={() => navigator.clipboard.writeText(label)}
            title={`${label} — click to copy`}>
              <span className="grad-sw-color checker" style={{ '--swatch-color': css }} />
              <span className={'grad-sw-hex fmt-' + stopFormat}>{label}</span>
            </button>);
      ...
      <div className="grad-actions">
        ...
        <button className="btn" onClick={copySteps}>copy steps</button>
        <button className="btn btn-ghost" onClick={() => onSave && onSave(stops)} title="Save current stops to library">☆ save</button>
      </div>
    </section>);
...
function ImageExtractor({ onPick, format, onSave }) {
  ...
  useE(() => {
    if (!src) { pickCanvasRef.current = null; return; }
    const img = new Image();
    img.onload = () => {
  ...
  const extract = useC((imageEl, k) => {
    ...
    while (cents.length < k) cents.push({ ...pts[Math.floor(rng() * pts.length)] });
    ...
    for (let it = 0; it < 12; it++) {
      ...
      for (const p of pts) {
        ...
        for (let j = 0; j < cents.length; j++) {
          const dL = p.L - cents[j].L,da = p.a - cents[j].a,db = p.b - cents[j].b;
          const d = dL * dL + da * da + db * db;
          if (d < bd) {bd = d;best = j;}
        }
        sums[best].L += p.L;sums[best].a += p.a;sums[best].b += p.b;sums[best].n++;
      }
      for (let j = 0; j < cents.length; j++) {
        if (sums[j].n > 0) {
          cents[j] = { L: sums[j].L / sums[j].n, a: sums[j].a / sums[j].n, b: sums[j].b / sums[j].n };
        }
      }
    }
    // count for sort
    ...
    const out = cents.map((c, i) => ({ ...ColorUtil.oklabToRgb(c.L, c.a, c.b), w: counts[i] })).
    sort((a, b) => b.w - a.w);
    return out;
  }, []);
  ...
  useE(() => {
    if (!eyedrop) return;
    const onKey = (e) => { if (e.key === 'Escape') { setEyedrop(false); setHover(null); } };
  ...
  return (
    <section className="panel">
        ...
        <div className="img-wrap">
            ...
            {eyedrop && hover &&
              <div
                ...
                <span className="eyedrop-ring" style={{ '--swatch-color': hover.css }} />
                <span className="eyedrop-tag">{hover.hex}</span>
              </div>
            }
          </div> :

        <div className="dz-empty">
            <div className="dz-text">drop image or click to upload</div>
          </div>
        }
        <input
          ref={fileInputRef}
      ...
      {src &&
        <div className="grad-actions eyedrop-bar">
          <button
            className={'btn' + (eyedrop ? ' btn-on' : '')}
            onClick={() => setEyedrop(v => !v)}
            title="Click anywhere on the image to set the current color">
            <span className="eyedrop-glyph" aria-hidden="true">⊙</span>
            {eyedrop ? 'picking — click image' : 'pick from image'}
          </button>
          <button
          ...
          <span className="eyedrop-hint">
            {eyedrop ? 'esc / toggle off to stop' : ''}
          </span>
        </div>
      }
      ...
      <>
          <div className="grad-swatches">
            ...
            return (
              <button key={i} className="grad-sw" onClick={() => onPick(hex)} title={hex}>
                  <span className="grad-sw-color checker" style={{ '--swatch-color': css }} />
                  <span className={'grad-sw-hex fmt-' + (format || 'hex')}>{label}</span>
                </button>);
          ...
          <div className="grad-actions">
            ...
            <button className="btn btn-ghost" onClick={() => onSave && onSave(palette)}>☆ save</button>
            <button className="btn btn-ghost" onClick={() => setSrc(null)}>clear</button>
          </div>
        </>
      }
    </section>);
...
function Library({ items, onLoadGradient, onLoadPalette, onRename, onRemove, onPick, onImport, onExport, format }) {
  ...
  const ioControls = (
    <div className="lib-io">
      <input
        ...
        onChange={onFile}
        hidden />
      <button className="lib-io-btn" onClick={triggerImport} title="Import library from a .json file">
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden="true">
          <path d="M6 1.5v6m0 0L3.5 5M6 7.5L8.5 5M2 9.5h8" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
        import
      </button>
      <button
        ...
        <svg width="12" height="12" viewBox="0 0 12 12" fill="none" aria-hidden="true">
          <path d="M6 7.5v-6m0 0L3.5 4M6 1.5L8.5 4M2 9.5h8" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
        export
      </button>
    </div>
      ...
      <section className="panel library-panel">
        ...
        <div className="lib-empty">
          ...
          <div className="lib-empty-text">
            No saved items yet. Use <em>save</em> in Harmonies, Gradient,
            or Image palette to build your library — or <em>import</em> a previously
            exported <code>.json</code> file.
          </div>
        </div>
        {msg && <div className={'lib-msg lib-msg-' + msg.kind}>{msg.text}</div>}
      </section>
    );
  }
  ...
  return (
    <section className="panel library-panel">
      <header className="panel-h">
        <h3>Library <span className="lib-count">{items.length}</span></h3>
        <div className="lib-header-tools">
          <div className="seg">
            {[
              { v: 'all', l: 'All' },
              { v: 'gradient', l: 'Gradients' },
              { v: 'palette', l: 'Palettes' }
            ].map(o => (
              <button key={o.v} className={'seg-btn' + (filter === o.v ? ' on' : '')} onClick={() => setFilter(o.v)}>{o.l}</button>
            ))}
          </div>
          {ioControls}
        </div>
...
function LibraryItem({ item, onLoad, onRename, onRemove, onPick, format }) {
  ...
  const previewBg = item.type === 'gradient'
    ? `linear-gradient(to right, ${colors.map(c => ColorUtil.toCss(c)).join(', ')})`
    : null;
  ...
  return (
    <div className="lib-card">
      ...
      >
        {item.type === 'gradient' ? (
          <span className="lib-grad-bar" style={{ '--grad-stops': previewBg }} />
        ) : (
          <span className="lib-pal-row">
            {colors.map((c, i) => (
              <span
                key={i}
                className="lib-pal-sw checker"
                style={{ '--swatch-color': ColorUtil.toCss(c) }}
                onClick={(e) => {
                  e.stopPropagation();
      ...
      <div className="lib-meta">
        {editing ? (
          <input
            ...
            onBlur={() => { onRename(name.trim() || item.name); setEditing(false); }}
            onKeyDown={(e) => {
              if (e.key === 'Enter') { onRename(name.trim() || item.name); setEditing(false); }
              if (e.key === 'Escape') { setName(item.name); setEditing(false); }
            }}
          />
        ) : (
          <button className="lib-name" onClick={() => setEditing(true)} title="Rename">
            {item.name}
          </button>
        )}
        <div className="lib-meta-row">
          <span className="lib-type">
            {item.type === 'gradient'
              ? `gradient · ${colors.length} stops`
              : item.meta && item.meta.type === 'harmony'
                ? `${item.meta.mode === 'split' ? 'split-comp' : 'analogous'} · ${colors.length} · ${item.meta.spread}°`
      ...
      <button className="lib-x" onClick={onRemove} title="Delete">×</button>
    </div>
  );
}
...
function App() {
  ...
  useE(() => {safeSet('cw-format', colorFormat);}, [colorFormat]);
  ...
  const [gradStops, setGradStops] = useS(() => [
    withStopId({ r: 245, g: 234, b: 215, a: 1 }),
    withStopId({ r: 38, g: 70, b: 83, a: 1 })
  ]);

  // Library (palettes + gradients), persisted.
  const [library, setLibrary] = useS(() => {
    try { return JSON.parse(safeGet('cw-library') || '[]'); }
    catch (e) { return []; }
  });
  useE(() => { safeSet('cw-library', JSON.stringify(library)); }, [library]);
  ...
  const savePalette = (input) => {
    ...
    let colors, meta = null;
    if (Array.isArray(input)) {
      colors = input;
    } else if (input && Array.isArray(input.colors)) {
      const { colors: c, ...rest } = input;
      colors = c;
      meta = rest;
    } else {
      return;
    ...
    const copy = colors.map(c => ({ r: c.r, g: c.g, b: c.b, a: c.a == null ? 1 : c.a }));
    const isHarmony = meta && meta.type === 'harmony';
    const baseName = isHarmony
      ? `${meta.mode === 'split' ? 'Split-comp' : 'Analogous'} ${countByType('palette') + 1}`
      : `Palette ${countByType('palette') + 1}`;
    setLibrary(L => [
      { id: uid(), type: 'palette', name: baseName, colors: copy, meta, created: Date.now() },
      ...L
    ]);
  };
  const saveGradient = (stops) => {
    if (!stops || stops.length < 2) return;
    const copy = stops.map(c => ({ r: c.r, g: c.g, b: c.b, a: c.a == null ? 1 : c.a }));
    setLibrary(L => [
      { id: uid(), type: 'gradient', name: `Gradient ${countByType('gradient') + 1}`, stops: copy, created: Date.now() },
      ...L
    ]);
  };
  const renameLib = (id, name) => setLibrary(L => L.map(it => it.id === id ? { ...it, name } : it));
  ...
  const normColor = (c) => {
    ...
    return { r: clamp(r), g: clamp(g), b: clamp(b), a: Number.isFinite(a) ? a : 1 };
  };
  const sigOf = (item) => {
  ...
  const validateItem = (raw) => {
    ...
    if (raw.type === 'gradient') {
      ...
      return { id: uid(), type: 'gradient', name: String(raw.name || `Gradient`), stops, created: Number(raw.created) || Date.now() };
    }
    if (raw.type === 'palette') {
      ...
      return { id: uid(), type: 'palette', name: String(raw.name || `Palette`), colors, meta, created: Number(raw.created) || Date.now() };
    }
    return null;
  };
  const importLibrary = (data) => {
    ...
    if (!raw) return { added: 0, skipped: 0, renamed: 0 };
    const incoming = raw.map(validateItem).filter(Boolean);
    if (incoming.length === 0) return { added: 0, skipped: 0, renamed: 0 };
    ...
    for (const it of incoming) {
      ...
      if (sigToIdx.has(s)) {
        ...
        if (next[idx].name !== it.name) {
          next[idx] = { ...next[idx], name: it.name };
          renamed++;
        } else {
          skipped++;
        }
  ...
  const loadGradient = (item) => setGradStops(item.stops.map(c => withStopId({ ...c, id: undefined })));
  const loadPalette = (item) => {
    ...
    if (item.meta && item.meta.type === 'harmony') {
      const m = item.meta;
      if (m.baseHsv) setHsv({ h: m.baseHsv.h, s: m.baseHsv.s, v: m.baseHsv.v });
    ...
    if (item.colors.length >= 2) setGradStops(item.colors.map(c => withStopId({ ...c, id: undefined })));
  };

  const [hsv, setHsv] = useS({ h: 18, s: 0.62, v: 0.92 });
  ...
  useE(() => { if (!hexFocused.current) setHexInput(hex); }, [hex]);
  ...
  return (
    <div className="app">
      <header className="topbar">
        <div className="brand">
          <span className="brand-mark checker" style={{ '--swatch-color': cssColor }} />
          <span className="brand-title">Color&nbsp;Workbench</span>
        </div>
        <div className="topbar-right">
          <Dropdown
            value={fontSize}
            onChange={setFontSize}
            leading={<span className="fs-aa">Aa</span>}
            width={140}
            options={[
      ...
      <main className="main">
        ...
        <section className="panel picker-panel">
          <header className="panel-h"><h3>Picker</h3></header>
          <div className="picker-stack">
            <div className="wheel-wrap">
              <HueWheel
                hue={hsv.h}
                onChange={(h) => setHsv((s) => ({ ...s, h }))}
                size={260}
              ...
              <div className="sv-overlay">
                <SVSquare
                  ...
                  onChange={(s, v) => setHsv((cur) => ({ ...cur, s, v }))}
                  size={136} />
                
              </div>
            </div>
            <HueStrip hue={hsv.h} onChange={(h) => setHsv((s) => ({ ...s, h }))} />
            <div className="row-2">
              <label className="lbl"><span>Hue</span><span className="lbl-val">{Math.round(hsv.h)}°</span></label>
              <input type="range" min={0} max={360} value={hsv.h}
              onChange={(e) => setHsv((s) => ({ ...s, h: Number(e.target.value) }))} />
            </div>
            <div className="row-2">
              <label className="lbl"><span>Saturation</span><span className="lbl-val">{Math.round(hsv.s * 100)}%</span></label>
              <input type="range" min={0} max={100} value={hsv.s * 100}
              onChange={(e) => setHsv((s) => ({ ...s, s: Number(e.target.value) / 100 }))} />
            </div>
            <div className="row-2">
              <label className="lbl"><span>Brightness (V)</span><span className="lbl-val">{Math.round(hsv.v * 100)}%</span></label>
              <input type="range" min={0} max={100} value={hsv.v * 100}
              onChange={(e) => setHsv((s) => ({ ...s, v: Number(e.target.value) / 100 }))} />
            </div>
            <div className="row-2">
              <label className="lbl"><span>Alpha</span><span className="lbl-val">{Math.round(alpha * 100)}%</span></label>
              <div className="alpha-track" style={{
                ...
                <input type="range" min={0} max={100} value={alpha * 100}
                  onChange={(e) => setAlpha(Number(e.target.value) / 100)} />
              </div>
            </div>
            <div className="hexinput-wrap hexinput-picker">
              <span className="hexinput-label">HEX</span>
              <input
                className="hexinput"
                value={hexInput}
        ...
        {/* CENTER: current swatch + outputs + history + harmonies */}
        <section className="center-col">
          <div className="big-swatch checker" style={{ '--swatch-color': cssColor }}>
            <div className="bs-meta">
              <span className="bs-label">current</span>
              <span className="bs-hex">{hex}</span>
            </div>
          </div>

          <ColorOutputs rgb={rgb} format={colorFormat} setFormat={setColorFormat} />
          ...
          <div className="history">
              ...
              <div className="hist-row">
                {history.map((h, i) =>
              <button key={i} className="hist-chip checker" style={{ '--swatch-color': h }} onClick={() => setFromHex(h)} title={h} />
              )}
              </div>
            </div>
          }
        ...
        {/* RIGHT: named colors */}
        <NamedColors onPick={setFromHex} currentHex={hex} />
      </main>

      <section className="tools">
        <ContrastChecker currentRgb={rgb} currentHex={hex} onPick={setFromHex} />
        <ToneScale currentRgb={rgb} format={colorFormat} onPick={setFromHex} onSave={savePalette} />
      </section>
      ...
      <section className="bottom">
        <Gradient
          ...
          onSave={saveGradient}
        />
        <ImageExtractor onPick={setFromHex} format={colorFormat} onSave={savePalette} />
      </section>
      ...
      <footer className="foot">
        <span>click any swatch to set current · click any code to copy</span>
      </footer>
    </div>);
